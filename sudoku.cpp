...
bool Sudoku::solveHelper(int row, int col) {
  ...
  if(row== 9){ // all complete
    return true;
  }

 //BASE CASES
  if(board[row][col] != 0){ // does not need to be solved 
    if(col == 8){ // reach end of column
...
//iterate through all num options
  for(int i = 1; i <= 9; i++){
    board[row][col] = i; 

    if(isValid(row, col)){ // if placement valid
